...
import { ArrowRight, ArrowLeft, ChevronDown, Sparkles, Zap, Shield } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 overflow-hidden">
        <div className="absolute top-1/4 right-1/4 w-[600px] h-[600px] bg-gradient-to-br from-primary/25 to-primary/5 rounded-full blur-3xl animate-pulse" style={{ animationDuration: '4s' }} />
        <div className="absolute bottom-1/4 left-1/4 w-[500px] h-[500px] bg-gradient-to-tr from-primary/15 to-transparent rounded-full blur-3xl animate-pulse" style={{ animationDuration: '6s' }} />
        ...
        <div className="absolute top-20 left-20 w-20 h-20 border border-primary/20 rounded-2xl rotate-12 animate-pulse" style={{ animationDuration: '3s' }} />
        <div className="absolute bottom-40 right-20 w-16 h-16 border border-primary/15 rounded-full animate-pulse" style={{ animationDuration: '4s' }} />
        <div className="absolute top-1/3 right-1/4 w-12 h-12 bg-primary/10 rounded-xl rotate-45" />
      </div>

      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-28 pb-20">
        <div className="text-center max-w-5xl mx-auto">
          <div className={`inline-flex items-center gap-3 px-6 py-3 rounded-full bg-gradient-to-r from-primary/15 via-primary/10 to-primary/15 border border-primary/25 mb-12 backdrop-blur-sm shadow-lg shadow-primary/5 ${isRTL ? "font-arabic" : ""}`}>
          ...
          <div className={`flex flex-col sm:flex-row items-center justify-center gap-5 mb-16 ${isRTL ? "sm:flex-row-reverse" : ""}`}>
            <Button
              size="lg"
            ...
            >
              {t("hero.cta.primary")}
              <ArrowIcon className={`h-5 w-5 transition-transform duration-300 group-hover:${isRTL ? "-translate-x-1.5" : "translate-x-1.5"}`} />
            </Button>
          ...
          <div className={`flex flex-wrap items-center justify-center gap-6 sm:gap-10 text-sm ${isRTL ? "flex-row-reverse" : ""}`}>
            ...
            <div className={`flex items-center gap-3 px-4 py-2 rounded-full bg-card/50 border border-border/50 backdrop-blur-sm ${isRTL ? "flex-row-reverse" : ""}`}>
              ...
              <span className={`text-muted-foreground ${isRTL ? "font-arabic" : ""}`}>{isRTL ? "استشارة مجانية" : "Free Consultation"}</span>
            </div>
            <div className={`flex items-center gap-3 px-4 py-2 rounded-full bg-card/50 border border-border/50 backdrop-blur-sm ${isRTL ? "flex-row-reverse" : ""}`}>
              <Shield className="w-4 h-4 text-primary" />
              <span className={`text-muted-foreground ${isRTL ? "font-arabic" : ""}`}>{isRTL ? "ضمان الجودة" : "Quality Guaranteed"}</span>
            </div>
          </div>
        </div>
